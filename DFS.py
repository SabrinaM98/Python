...
class Grafo:
    ...
    def percorre_negada(self, origem):
        # coloca o destino
        #adicionar um condicional no etste do vertice para ele retornat o count caso positivo
        vertice = self.busca_vertice(origem)

        if not vertice:
            return 'vertice n exite bro'
...
#    print(grafo.DFS_lindo(vertices[0]))

#   

    print(grafo.percorre_negada(vertices[0], vertices[-3]))

'''    print('Qual vertice será a origem?\n')
    n = int(input())
    print('\n')
    index = vertices.index(n)
    if n not in vertices:
        print('Vertice invalido')
    else:
        print('///Printando a distancia do vertice {} até cada vertice do grafo///\n'.format(n))
        print(grafo.percorre_negada(vertices[index]))'''
